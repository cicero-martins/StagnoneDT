"""
Per-deploy drifter figures for v04AE (full 9d, AE-only blend, D-Morph ON).

For each of the 12 July 2025 deploys, plot a 2-panel figure:
  - left:  Stagnone overview with all 3 drifters of the deploy (obs solid, sim dashed)
  - right: zoomed tracks for the same deploy

Output: figures/v04AE_drifter_deploy_NN.png (NN = 01..12)

Reuses: drifter_tracks_Jul2025.csv (obs), drifter_sim_v04AE.csv (sim).
Land masking: same v04_land_mask_50m.nc + sicily2.ldb / Stagnone_dxy01_15m.ldb
pair used in notebook 32 for v04rAE plots.
"""
...
VARIANT = 'v04AE'
TITLE_TAG = 'v04AE, full 9d Jul 1-10, AE-only blend, D-Morph ON'
...
def main():
    ...
    if land_mask_path.exists():
        ...
        print(f'  land mask loaded: {land_arr.shape}')
    else:
        print('  WARN no land mask; will skip raster fill')
        land_lon = land_lat = land_arr = None
