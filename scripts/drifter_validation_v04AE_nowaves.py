"""Drifter validation for v04AE_nowaves (Jul 1-10, waves OFF), end-to-end.

Mirror of [drifter_validation_v04AE_nodm.py] with paths and labels swapped.

This is the fifth member of the Paper 1 ensemble. The contrast that matters is
nowaves vs nodm: the two share everything (fixed bed, uniform roughness, same
forcing, same initial condition) except wave coupling, so the difference in LW
skill is the wave contribution.

WINDAGE is held at 0.02 across every ensemble member on purpose. The July 2025
sweep found no single optimal value (interior deploys prefer 0.02, the southern
margin prefers 0.005), but varying it between members would confound the
process attribution with a tuning difference.

Prerequisite: regrid_v04AE_nowaves_for_opendrift.py, run on simit-server, and
the resulting v04AE_nowaves_surface_current.nc copied into data/processed/.
"""
...
REGRID = PROC / 'v04AE_nowaves_surface_current.nc'
RUN = 'v04AE_nowaves'

WINDAGE = 0.02

# Ensemble members to tabulate alongside. nodm is the direct counterpart.
ENSEMBLE = ['v04AE_nodm', 'v04AE_nodm_vr', 'v04AE', 'v04AE_vr']
...
def main():
    assert REGRID.exists(), (
        f'Missing {REGRID}. Run regrid_v04AE_nowaves_for_opendrift.py on '
        f'simit-server and copy the output here first.')
    ...
    out_nc = PROC / f'opendrift_{RUN}.nc'
    ...
    sim_csv = PROC / f'drifter_sim_{RUN}.csv'
    ...
    metrics_csv = PROC / f'drifter_metrics_{RUN}.csv'
    metrics_df.to_csv(metrics_csv, index=False)
    print(f'\nScored {len(metrics_df)} tracks. Saved {metrics_csv}')

    if not len(metrics_df):
        return
    ...
    cmp = metrics_df.rename(columns={'endpoint_sep_m': f'EP_{RUN}',
                                     'LW_skill': f'LW_{RUN}'})
    cmp = cmp[['deploy', 'drifter_id', f'EP_{RUN}', f'LW_{RUN}']]
    for label in ENSEMBLE:
        p = PROC / f'drifter_metrics_{label}.csv'
        if not p.exists():
            print(f'  (missing {p.name}, skipped in comparison)')
            continue
    ...
    per = cmp.groupby('deploy').agg({
        c: 'mean' for c in cmp.columns if c.startswith(('EP_', 'LW_'))
    }).round(2)
    ...
    print(per[cols].to_string())

    print('\n=== Global means ===')
    for label in [RUN] + ENSEMBLE:
        col, ep_col = f'LW_{label}', f'EP_{label}'
        if col in cmp.columns:
            print(f'  {label:15s}: LW={cmp[col].mean():.3f}  '
                  f'EP={cmp[ep_col].mean():.0f} m  n={cmp[col].notna().sum()}')

    if f'LW_v04AE_nodm' in cmp.columns:
        dlw = cmp['LW_v04AE_nodm'].mean() - cmp[f'LW_{RUN}'].mean()
        dep = cmp['EP_v04AE_nodm'].mean() - cmp[f'EP_{RUN}'].mean()
        print(f'\n=== Wave contribution (nodm minus nowaves, identical otherwise) ===')
        print(f'  LW {dlw:+.3f}   EP {dep:+.0f} m')
        print(f'  Positive LW means wave coupling improves Lagrangian skill.')

    cmp_csv = PROC / f'drifter_compare_{RUN}.csv'
