"""
Drifter validation for v04AE (full 9-day Jul 1-10, AE-only wind blend, D-Morph ON).

End-to-end:
  1. Loads data/processed/v04AE_surface_current.nc (made by regrid_v04AE_for_opendrift)
  2. Reads drifter_releases_Jul2025.csv (12 deploys, all Jul 8)
  3. Runs OpenDrift OceanDrift with windage=0.02 (matches v04rAE comparison)
  4. Saves opendrift_v04AE.nc, drifter_sim_v04AE.csv, drifter_metrics_v04AE.csv
  5. Prints comparison table with v04 / v04r / v04rE5 / v04rAE

Skill metrics per drifter:
  - endpoint_sep_m: terminal position error
  - path_ratio: sim path length / obs path length
  - LW_skill: Liu-Weisberg (1 - normalized cumulative separation)
"""
...
REGRID = PROC / 'v04AE_surface_current.nc'
...
def main():
    assert REGRID.exists(), f'Missing {REGRID} — run scripts/regrid_v04AE_for_opendrift.py first'
    ...
    out_nc = PROC / 'opendrift_v04AE.nc'
    ...
    # --- 2. Extract per-trajectory positions
    ...
    metrics_csv = PROC / 'drifter_metrics_v04AE.csv'
    ...
    if len(metrics_df):
        ...
        for label, fn in [('v04', 'drifter_metrics_v04.csv'),
            ...
            if p.exists():
                ...
                rows.append(tmp)
        cmp = metrics_df.rename(columns={'endpoint_sep_m': 'EP_v04AE',
                                          'LW_skill': 'LW_v04AE'})
        cmp = cmp[['deploy', 'drifter_id', 'EP_v04AE', 'LW_v04AE']]
        for tmp in rows:
        ...
        print('\n=== Global means ===')
        for label in ['v04', 'v04r', 'v04rE5', 'v04rAE', 'v04AE']:
            col = f'LW_{label}'
            ep_col = f'EP_{label}'
            if col in cmp.columns:
                print(f'  {label:7s}: LW={cmp[col].mean():.3f}  '
                      f'EP={cmp[ep_col].mean():.0f} m  '
                      f'n={cmp[col].notna().sum()}')

        for ref in ['v04rAE', 'v04rE5', 'v04r', 'v04']:
            ref_lw = f'LW_{ref}'
            if ref_lw in cmp.columns:
                dlw = cmp['LW_v04AE'].mean() - cmp[ref_lw].mean()
                print(f'  Delta v04AE vs {ref:6s}:  LW {dlw:+.3f}')

        # Save the comparison table
        cmp_csv = PROC / 'drifter_compare_v04AE_all.csv'
