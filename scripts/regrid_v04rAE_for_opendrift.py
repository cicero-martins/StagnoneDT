"""
Regrid v04rAE surface currents (Jul 8-9 re-run from restart at Jul 8 00:00,
mapInterval = 600 s) onto a rectilinear lat/lon grid for OpenDrift.

Identical structure to regrid_v04_for_opendrift.py, except:
  - reads from model/dflowfm_v04rAE/DFM_OUTPUT_*
  - time slice is Jul 8 00:00 -> Jul 10 00:00 (the v04rAE run window)
  - output: data/processed/v04rAE_surface_current.nc
...
Run:
    python scripts/regrid_v04rAE_for_opendrift.py
"""
...
V04R = ROOT / 'model' / 'dflowfm_v04rAE' / 'DFM_OUTPUT_Stagnone_dxy01_15m'
OUT = ROOT / 'data' / 'processed' / 'v04rAE_surface_current.nc'
...
# v04rAE ran Jul 8-9 only; start at the very beginning of the restart window
...
def main():
    ...
    # v04rAE: use the AE-only blended wind (same as FM was forced with).
    # OpenDrift reads these as x_wind/y_wind and applies 2% windage. For
    # consistency with the v04rAE FM run, OpenDrift sees the same forcing.
    print('\nAdding wind (u10, v10) from AE-only blend (consistent with v04rAE FM forcing)...')
    wind_u_path = ROOT / 'model' / 'dflowfm_v04rAE' / 'wind_blendedAE_u10n_20250701to20250710.nc'
    wind_v_path = ROOT / 'model' / 'dflowfm_v04rAE' / 'wind_blendedAE_v10n_20250701to20250710.nc'
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            'Conventions': 'CF-1.8',
            'source': 'Regridded from Stagnone v04rAE (8 MPI partitions, mapInterval=600s) + blended wind',
