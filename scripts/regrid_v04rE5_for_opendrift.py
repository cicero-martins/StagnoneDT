"""
Regrid v04rE5 surface currents (Jul 8-9 re-run from restart at Jul 8 00:00,
mapInterval = 600 s) onto a rectilinear lat/lon grid for OpenDrift.

Identical structure to regrid_v04_for_opendrift.py, except:
  - reads from model/dflowfm_v04rE5/DFM_OUTPUT_*
  - time slice is Jul 8 00:00 -> Jul 10 00:00 (the v04rE5 run window)
  - output: data/processed/v04rE5_surface_current.nc

Same land masking (FM dry faces with bl >= -0.05 m dropped) and same wind blend
(ERA5+SiAM blended u10n/v10n packed into the same file under CF standard names).

Run:
    python scripts/regrid_v04rE5_for_opendrift.py
"""
...
V04R = ROOT / 'model' / 'dflowfm_v04rE5' / 'DFM_OUTPUT_Stagnone_dxy01_15m'
OUT = ROOT / 'data' / 'processed' / 'v04rE5_surface_current.nc'
...
# v04rE5 ran Jul 8-9 only; start at the very beginning of the restart window
T_MIN = '2025-07-08T00:00:00'
T_MAX = '2025-07-10T00:00:00'


def main():
    map_pattern = str(V04R / 'Stagnone_dxy01_15m_0*_map.nc')
    ...
    print(f'Subset time: {times[0]} .. {times[-1]} ({len(times)} snapshots, vs ~96 in v04)')
    ...
    for it in range(ntime):
        ...
        if (it + 1) % 50 == 0 or it == ntime - 1:
            print(f'  [{it+1:3d}/{ntime}] {times[it]}  u_abs_max={np.nanmax(np.abs(u_flat)):.3f} m/s')

    # v04rE5: use ERA5 raw wind (same as FM was forced with), NOT the blended file.
    # OpenDrift will read these as x_wind/y_wind and apply 2% windage. For
    # consistency with the v04rE5 FM run, OpenDrift sees the same forcing.
    print('\nAdding wind (u10, v10) from ERA5 raw (consistent with v04rE5 FM forcing)...')
    wind_u_path = ROOT / 'model' / 'dflowfm_v04rE5' / 'wind_era5raw_u10n_20250701to20250710.nc'
    wind_v_path = ROOT / 'model' / 'dflowfm_v04rE5' / 'wind_era5raw_v10n_20250701to20250710.nc'
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            'Conventions': 'CF-1.8',
            'source': 'Regridded from Stagnone v04rE5 (8 MPI partitions, mapInterval=600s) + blended wind',
            'description': 'Surface currents + 10-m wind, rectilinear grid for OpenDrift, Jul 8-9 2025',
        },
    )
