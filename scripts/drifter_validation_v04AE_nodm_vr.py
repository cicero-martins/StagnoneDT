"""Drifter validation for v04AE_nodm_vr (Jul 1-10 D-Morph OFF, VR ON), end-to-end.
...
REGRID = PROC / 'v04AE_nodm_vr_surface_current.nc'
...
def main():
    assert REGRID.exists(), f'Missing {REGRID} — run regrid_v04AE_nodm_vr_for_opendrift.py first'
    ...
    out_nc = PROC / 'opendrift_v04AE_nodm_vr.nc'
    ...
    sim_csv = PROC / 'drifter_sim_v04AE_nodm_vr.csv'
    ...
    metrics_csv = PROC / 'drifter_metrics_v04AE_nodm_vr.csv'
    metrics_df.to_csv(metrics_csv, index=False)
    print(f'\nScored {len(metrics_df)} tracks. Saved {metrics_csv}')

    if len(metrics_df):
        print(f'\nv04AE_nodm_vr summary:')
        ...
        for label, fn in [('v04AE', 'drifter_metrics_v04AE.csv'),
            ...
            if p.exists():
                ...
                rows.append(tmp)
        cmp = metrics_df.rename(columns={'endpoint_sep_m': 'EP_v04AE_nodm_vr',
                                          'LW_skill': 'LW_v04AE_nodm_vr'})
        cmp = cmp[['deploy', 'drifter_id', 'EP_v04AE_nodm_vr', 'LW_v04AE_nodm_vr']]
        for tmp in rows:
        ...
        print('\n=== Global means ===')
        for label in ['v04rAE', 'v04AE', 'v04AE_nodm_vr']:
        ...
        for ref in ['v04AE', 'v04rAE']:
            ...
            if ref_lw in cmp.columns:
                dlw = cmp['LW_v04AE_nodm_vr'].mean() - cmp[ref_lw].mean()
                dep = cmp['EP_v04AE_nodm_vr'].mean() - cmp[f'EP_{ref}'].mean()
                print(f'  Delta v04AE_nodm_vr vs {ref:7s}: LW {dlw:+.3f}  EP {dep:+.0f} m')

        cmp_csv = PROC / 'drifter_compare_v04AE_nodm_vr.csv'
