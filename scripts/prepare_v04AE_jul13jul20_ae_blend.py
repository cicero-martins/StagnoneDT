"""Build wind_blendedAE_{u10n,v10n}_20250713to20250721.nc for v04AE Opt-A
cold-start Jul 13-21 window.

Same blend logic as prepare_v04AE_d10d12_ae_blend.py:
  - Inside 3 km of lagoon centre: AE in-situ only
  - Beyond 8 km: ERA5 only
  - Linear taper in between

Source files:
  - ERA5 raw: model/dflowfm_v04AE_jul13jul20/wind_era5raw_*_20250713to20250721.nc
  - AE in-situ: data/processed/insitu_2025-26/AE_wind_UTC.csv
"""
...
def main():
    era5_u_path = DST / 'wind_era5raw_u10n_20250713to20250721.nc'
    era5_v_path = DST / 'wind_era5raw_v10n_20250713to20250721.nc'
    for p in (era5_u_path, era5_v_path):
        if not p.exists():
            raise FileNotFoundError(
                f'ERA5 raw not found at {p}. '
                'Run scripts/preprocess_era5_v04AE_jul13jul20.py first.')
    ...
    for it in range(len(times)):
        ...
        if (it + 1) % 48 == 0 or it == len(times) - 1:
            cl = len(lat_out) // 2
            cn = len(lon_out) // 2
    ...
    def save(name, data, var, std_name, long_name):
        ds = xr.Dataset(
            ...
            attrs={
                ...
                'history': 'Created by scripts/prepare_v04AE_jul13jul20_ae_blend.py',
            },
        )
        out = DST / name
    ...
    save('wind_blendedAE_u10n_20250713to20250721.nc', u_out, 'u10n',
         'eastward_wind', '10 m eastward wind, AE-only blend (Jul 13-21 2025)')
    save('wind_blendedAE_v10n_20250713to20250721.nc', v_out, 'v10n',
         'northward_wind', '10 m northward wind, AE-only blend (Jul 13-21 2025)')
